...
export default function Itemperature(props: {
  dataDef: ItemperatureProps;
  edit?: boolean
  delete?: boolean
}) {
  ...
  useEffect(() => {
    if (deleteMeasure == "y") {
      ...
      DefaultAllData.deleteMeasurement(id_measure,"temperature").then((res) => {
  ...
  const handleSubmit = (event: {
    ...
    let inputValue = event.target.temperature.value;
    if (inputValue == null) {
      setDataError(true);
      ...
      if (dataDisabled == false) {
        ...
        let patientId = localStorage.getItem("IdPatient");
        let value1 = event.target.temperature.value;
    ...
    <>
      ...
      <form onSubmit={handleSubmit} id='my-form'>
        <Box display="flex"
          ...
          alignItems="center"> <Box component="img" src="/static/measure_patient/Temperature.png" alt="logo" /></Box>
