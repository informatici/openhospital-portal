...
export default function Isaturation(props: {
  dataDef: IsaturationProps;
  edit?: boolean
  delete?: boolean
}) {
  ...
  useEffect(() => {
    if (deleteMeasure == "y") {
      ...
      DefaultAllData.deleteMeasurement(id_measure,"stauration").then((res) => {
  ...
  const handleSubmit = (event: {
    ...
    let inputValue = event.target.saturation.value;
    if (inputValue == null) {
      setDataError(true);
      ...
      if (dataDisabled == false) {
        ...
        let patientId = localStorage.getItem("IdPatient");
        let value1 = event.target.saturation.value;
    ...
    <>
      ...
      <form onSubmit={handleSubmit} id='my-form'>
        <Box display="flex"
          ...
          alignItems="center"> <Box component="img" src="/static/measure_patient/Saturation.png" alt="logo" /></Box>
